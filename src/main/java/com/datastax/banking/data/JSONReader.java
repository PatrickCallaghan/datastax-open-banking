	...
	public JSONReader() {
		...
		try {
			...
			while (iterator.hasNext()) {
				...
				bankService.saveUser(new User(user.get("user_name").toString(), user.get("email").toString(),
						user.get("password").toString(), user.get("display_name").toString()));
			}
			...
			while (iterator.hasNext()) {
				...
				JSONArray owners = (JSONArray)account.get("owners");
				...
				for (int i = 0; i < owners.size(); i++) {
					String owner = owners.get(i).toString();
					
					Permission permission = new Permission(account.get("bank").toString(), account.get("id").toString(),
							owner, "PROVIDER_1", UUID.randomUUID().toString(),
							BankGenerator.getRandomPermissions());

					bankService.savePermission(permission);

					permission = new Permission(account.get("bank").toString(), account.get("id").toString(),
							owner, "PROVIDER_2", UUID.randomUUID().toString(),
							BankGenerator.getRandomPermissions());
					bankService.savePermission(permission);
				}
			}
			...
			while (iterator.hasNext()) {
				...
				bankService.saveBranch(new Branch(branch.get("bank_id").toString(), branch.get("id").toString(),
						branch.toJSONString()));
			}
			...
			while (iterator.hasNext()) {
				...
				bankService.saveAtm(
						new Atm(atm.get("bank_id").toString(), atm.get("id").toString(), latlon, atm.toJSONString()));
			}
			...
			while (iterator.hasNext()) {
				...
				bankService.saveProduct(new Product(product.get("bank_id").toString(), product.get("code").toString(),
			...
			while (iterator.hasNext()) {
				...
				bankService.saveTransacton(new Transaction(transactionId, completed, accountId, counterpartyName, value,
